...
jQuery.fn.bic_calendar = function (options) {
    ...
    this.each(function () {
        ...
        function showCalendar() {
            ...
            showMonths(year);
            var nextYearButton = jQuery('<td><a   class="button-year-next" id="loaddata"><span class="slip-icons"><img src="' + image_url + '/Arrow right.png"></span></a></td>');
            nextYearButton.click(function (e) {
                var currentYear = objFecha.getFullYear();
                var nextYear = year + 1;
                if (nextYear <= currentYear) {
                    jQuery(".ajax_calender_load").empty('');
                    wphrm_data = {
                        'action': 'WPHRMAjaxCalenderLoad',
                        'wphrm_empId': WPHRMEmployeeDatas.wphrmEmployeeID,
                        'wphrm_year': nextYear,
                    }
                    ...
                    jQuery.post(ajaxurl, wphrm_data, function (response) {
                        ...
                        jQuery(".ajax_calender_load").html(data.wphrmSalaryData);
                    });
                    e.preventDefault();
                    year++;
                    changeDate(year);
            ...
            var previousYearButton = jQuery('<td><a  class="button-year-previous"><span class="slip-icons"><img src="' + image_url + '/Arrow left.png"></span></a></td>');
            //event
            previousYearButton.click(function (e) {
                var prevYear = year - 1;
                var employeeJoiningYear = WPHRMEmployeeDatas.wphrmEmployeeJoiningYear;
                if (prevYear >= employeeJoiningYear) {
                    jQuery(".ajax_calender_load").empty('');
                    jQuery('.preloader-custom-gif').show();
                    jQuery('.preloader ').show();
                    wphrm_data = {
