...
Class WPHRM extends WPHRMConfig {
    ...
    public function WPHRMAssets() {
        ...
        //wp_register_script('wphrm-jaf-dt-buttons', plugins_url('assets/js/dt-lib/dataTables.buttons.min.js', __FILE__), '');
        //wp_register_script('wphrm-jaf-dt-jszip', plugins_url('assets/js/dt-lib/jszip.min.js', __FILE__), '');
        //wp_register_script('wphrm-jaf-dt-pdfmake', plugins_url('assets/js/dt-lib/pdfmake.min.js', __FILE__), '');
        //wp_register_script('wphrm-jaf-dt-vfs_fonts', plugins_url('assets/js/dt-lib/vfs_fonts.js', __FILE__), '');
        //wp_register_script('wphrm-jaf-dt-html5-button', plugins_url('assets/js/dt-lib/buttons.html5.min.js', __FILE__), '');
        ...
        //wp_register_style('wphrm-bootstrap-min-css', '//maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css', null, null);
        ...
        //wp_register_script('wphrm-jaf-dt-pdfmake', '//cdn.rawgit.com/bpampuch/pdfmake/0.1.24/build/pdfmake.min.js', array('wphrm-jquery-dataTables-min-js') );
        ...
        //wp_register_script('wphrm-jaf-dt-vfs_fonts', '//cdn.rawgit.com/bpampuch/pdfmake/0.1.24/build/vfs_fonts.js', array('wphrm-jquery-dataTables-min-js') );
    ...
    public function WPHRMMenu() {
        ...
        if (empty($wphrmViewOnlyPermissionPages)) {
            ...
            if (in_array('manageOptionsAttendances', $wphrmGetPagePermissions)) {
                ...
                $wphrmPage = add_submenu_page('wphrm', 'Attendance Management', __('Attendance Management', 'wphrm'), 'manageOptionsAttendancesView', 'wphrm-view-attendance', array(&$this, 'WPHRMViewAttendances'));
                add_action("admin_print_styles-{$wphrmPage}", array(&$this, 'WPHRMEnqueues'));
            }
            ...
            if (in_array('manageOptionsNotice', $wphrmGetPagePermissions)) {
                ...
                $wphrmPage = add_submenu_page('wphrm', 'Notices', __('Notices', 'wphrm'), 'manageOptionsNoticeView', 'wphrm-notice', array(&$this, 'WPHRMNoticeCallback'));
                add_action("admin_print_styles-{$wphrmPage}", array(&$this, 'WPHRMEnqueues'));
            }
    ...
    public function wphrmPageInAddCapability($wphrm_rolename, $pagename, $wphrmAll, $wphrmGetDefineCapabilityData) {
        $getRoles = get_role($wphrm_rolename);
        //wp_mail('aries@g.com', '$wphrmGetDefineCapabilityData', print_r($wphrmGetDefineCapabilityData,1));
        foreach ($pagename as $pageKey => $pagenames) {
            foreach ($wphrmAll as $wphrmAllKey => $wphrmAllpermission) {
                if ($pageKey == $wphrmAllKey) {
    ...
    public function wphrmAddMainMenu($wphrmManupages) {
        //        echo "<pre>";  print_r($wphrmManupages); exit();
        $NotificationCounters = '';
        $NotificationCounter = $this->WPHRMNotificationCounter();
        if ($NotificationCounter != '') {
            $NotificationCounters = $NotificationCounter;
    ...
    public function wphrmRoleWiseCapabilityGet() {
        ...
        //wp_mail('ar@sa.com', '$wphrmUserDefineCapabilities', print_r($wphrmUserDefineCapabilities,1));
    ...
    public function WPHRMChangeRoleWiseDisplay() {
        ...
        if (isset($_POST['wphrm-actions']) && $_POST['wphrm-actions'] == 'adddatabaserole') {
            ...
            if ($changeRole == 'yes') {
                ...
                /* $whereArray = array('authorID' => 'undefineaccess','settingKey' => $wphrmUserRole);
                  $datas = array('authorID' => 'defineaccess');
                  $wpdb->update($this->WphrmSettingsTable, $datas, $whereArray); */
            }
            ...
            if ($changeRole == 'no') {
                ...
                /* $whereArray = array('authorID' => 'defineaccess','settingKey' => $wphrmUserRole);
                  $datas = array('authorID' => 'undefineaccess');
                  $wpdb->update($this->WphrmSettingsTable, $datas, $whereArray); */
...
<?php
    ...
    public function WPHRMEmployeeBasicInfo() {
        ...
        if (isset($_POST['wphrm_employee_entitled_leave']) && $_POST['wphrm_employee_entitled_leave'] != '') {
            $wphrm_employee_entitled_leave = ($_POST['wphrm_employee_entitled_leave']);
        }
    ...
    * */
        ...
        if (!empty($wphrmLeaveApplicationId)) {
            ...
            if ($wphrmapplicationLeavedate != '' && $wphrmapplicationLeavedateTo != '') {
                ...
                if (isset($wphrmApplicationStatus) && $wphrmApplicationStatus == 'approved') {
                    ...
                    //$result['custom_message'] = implode(', ', $dateBetweenArray);
                    ...
                    //$this->WPHRMSendEmail($wphrmEmployeeID, 'approved', array('from' => date('d F Y', strtotime($wphrmapplicationLeavedate)), 'to' => date('d F Y', strtotime($wphrmapplicationLeavedateTo))));
                    ...
                    //$this->WPHRMSendEmail($wphrmEmployeeID, 'rejected', array('from' => date('d F Y', strtotime($wphrmapplicationLeavedate)), 'to' => date('d F Y', strtotime($wphrmapplicationLeavedateTo))));
    ...
    public function WPHRMNoticeInfo() {
        ...
        if (!empty($wphrmNotice)){
            ...
            /*$id = $wpdb->query("UPDATE $this->WphrmNoticeTable SET
...
*/
    ...
    function confirmNoticeInvitation(){
        ...
        if($notice_id){
            ...
            if($duplicate){
                ...
                $wpdb->update($this->WphrmInvitationAttendeeTable, $data, $where );
                $response['message'] = 'Status is updated to '.strtoupper($status);*/
    ...
    function get_employee_types(){
        $employee_levels = array(
            ...
            'staff' => array(
                'title' => 'Level 4 Warehouse/Driver/Technician(NE-III)',
                'entitlement' => 7,
                'cap' => 14,
                'entitle_medical_claim' => 300,
                'entitle_elderly_screening' => 0,
    ...
    function get_leave_rules($rule = null){
        $leave_rules = array(
            'year_of_service' => array( 'title' => '1 Year of Service', 'description' => 'Year of Service - Atleast 1 year of service to the company.' ),
            'regular_employee_leave' => array( 'title' => 'Regular Employee (After Probation Period)', 'description' => 'Regular Employee - After employee probation period, he/she therefore eligible to take this leave type.' ),
            'annual_leave' => array( 'title' => 'Annual Leave', 'description' => 'Annual Leave - Automatically calculated based on joining date & employee level. Can only be earned after probationary period.' ),
            'medical_leave' => array( 'title' => 'Medical Leave', 'description' => 'Medical Leave - Automatically calculated based on joining date. Also entitled for Medical Claims' ),
            'hospitality_leave' => array( 'title' => 'Hospitality Leave', 'description' => 'Set the Maximum here. if ML has been used, the HL will be deducted by the used ML.' ),
            'maternity_leave' => array( 'title' => 'Maternity Leave', 'description' => 'For Female only.' ),
            'paternity_leave' => array( 'title' => 'Paternity Leave', 'description' => 'For Male only. Must be a regular employee.' ),
            'urgent_leave' => array( 'title' => 'Urgent Leave', 'description' => 'This will be deducted to annual leave.' ),
            'compassionate_leave' => array( 'title' => 'Compasionate Leave', 'description' => 'Entitlement = 1 DAY - CRITICAL ILLNESS LEAVE (DANGEROUSLY ILL LIST CERTIFIED BY DOC) Entitlement = 3 DAYS – DEATH (GRANDPARENT, PARENT, PARENT IN LAW, SIBILINGS, SPOUSE,CHILD)' ),
            'national_service_leave' => array( 'title' => 'National Service Leave', 'description' => 'National Service Leave - employee must be a Male and after probation period.' ),
        );
        return $leave_rules;
    }

    function get_leave_rule($rule){
        $all_rules = $this->get_leave_rules();
        if(isset($all_rules[$rule])){
            return $all_rules[$rule];
        }else{
            return $rule;
        }
    }
    ...
    function get_employee_max_leave($user_id, $leave_rule, $leave_type = null, $year = null){
        ...
        //calculate the max leave for this employee
        //var_dump($leave_rule);
        switch($leave_rule){
            ...
            case 'annual_leave':
                {
                    ...
                    //if this is the first year give just entitlement count
                    /*$service_period = $this->get_employment_period($user_id);
            if($service_period->y <= 1){
                return $total;
                    ...
                    if(
                        ...
                        $total = $total + $annual_leave_history[$year-1]['count'];
                    }
    ...
    function get_used_leave($user_id, $leave_id, $year = null, $month_start = null, $month_end = null){
        ...
        $leaveday_count = 0.0;
        $employee_leaves = $wpdb->get_results("SELECT * FROM $this->WphrmLeaveApplicationTable WHERE `employeeID` ='" . $user_id . "' AND `leaveType` = '$leave_id' AND ( `date` >= '$year_start_date' AND `date` <= '$year_end_date' ) AND `applicationStatus`='approved'");
        ...
        foreach($employee_leaves as $leave){
            ...
            //at this point the leave day is greater than 1
...
/*if(!function_exists('jaf_get_user_leave_count')){
    function jaf_get_user_leave_count($id, $user_id){
        global $wpdb, $wphrm;
        $leave_counts = array( 'used' => 0, 'total' => 0 );
        $leave_info = $this->get_leave_info($id);
        $special_leaves = jaf_get_special_leaves();
        $leave_ruless = array_keys($special_leaves);
        if(in_array($leave_info->leave_rules, $leave_ruless)){
            switch($leave_info->leave_rules){
                case 'annual_leave':
            ...
            $leave_counts['used'] = 0;
            $leave_counts['total'] = $leave_info->numberOfLeave;
        }
    }
}*/
