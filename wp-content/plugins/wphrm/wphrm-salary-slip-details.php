...
if (isset($_REQUEST['wphrm-employee-id']) && !empty($_REQUEST['wphrm-employee-id']) && isset($_REQUEST['wphrm-create-salary-year']) && !empty($_REQUEST['wphrm-create-salary-year']) && isset($_REQUEST['wphrm-create-salary-month']) && !empty($_REQUEST['wphrm-create-salary-month'])) :
    $employee_id = esc_sql($_REQUEST['wphrm-employee-id']); // esc
    $generate_month = esc_sql($_REQUEST['wphrm-create-salary-month']); // esc
    $generate_year = esc_sql($_REQUEST['wphrm-create-salary-year']); // esc
    ...
    $duplicateMonth = '';
    $duplicateFrom = '';
    $wphrm_generated_salary = $wpdb->get_results("SELECT * FROM $this->WphrmSalaryTable WHERE `employeeID` = $employee_id AND date(`date`)='$from'");
    $wphrm_text_infos = $wpdb->get_row("SELECT * FROM $this->WphrmSalaryTable WHERE `employeeID` = $employee_id AND `employeeKey` = '$employeeSalaryNote' AND `date` ='$from'");
    if ((isset($_REQUEST['wphrm-duplicate-to-salary-year']) && ($_REQUEST['wphrm-duplicate-to-salary-year'] != '')) && (isset($_REQUEST['wphrm-duplicate-to-salary-month']) && ($_REQUEST['wphrm-duplicate-to-salary-month'] != ''))) {
        $duplicateMonth = esc_sql(date('m', strtotime($_REQUEST['wphrm-duplicate-to-salary-month'])));
        $duplicateFrom = esc_sql($_REQUEST['wphrm-duplicate-to-salary-year'] . '-' . $duplicateMonth . '-' . '01'); // esc
        $wphrm_generated_salary = $wpdb->get_results("SELECT * FROM $this->WphrmSalaryTable WHERE `employeeID` = $employee_id AND date(`date`)='$duplicateFrom'");
        $wphrm_text_infos = $wpdb->get_row("SELECT * FROM $this->WphrmSalaryTable WHERE `employeeID` = $employee_id AND `employeeKey` = '$employeeSalaryNote' AND `date` ='$duplicateFrom'");
    }
    $wphrm_text_info = '';
            ...
            <div id="myModaldelete" class="modal fade" tabindex="-1" data-backdrop="static" data-keyboard="false">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <form id="frm_salary_delete" class="search-form" method="post">
            ...
            <div class="col-md-12 title-padding">
                <div class="row back-row">
                    <div style="padding-left: 0px;" class="col-md-4">
                        <a href="?page=wphrm-select-financials-month&employee_id=<?php echo esc_attr($employee_id); ?>" class="btn blue"><i class="fa fa-arrow-left"></i><?php _e('Back', 'wphrm'); ?></a>
                    ...
                    <div class="col-md-4 salary-slip">
                        <h1><?php _e('Create Salary Slip of ' . $generate_month . ' ' . $generate_year, 'wphrm'); ?></h1>
                    </div>
                ...
                <div class="portlet-body">
                    <div class="row">
                        ...
                        <div class="slip-user-table">
                            ...
                            <div class="row">
                                <form id="frm_salary_generate" name="myform" class="search-form" method="post">
                                    ...
                                    <div class="">
                                        <?php if (isset($wphrmSalaryAccording['wphrm-according']) && $wphrmSalaryAccording['wphrm-according'] == 'Day') { ?>
                                            <div style="text-align: center;" class="col-md-3">
                                                ...
                                                if (isset($totalofWorkingDays)): echo esc_attr($totalofWorkingDays);
                                                endif;
                                                ?>">
                                            </div>
                                            <div style="text-align: center;" class="col-md-3">
                                                ...
                                                if (isset($presents)): echo esc_attr($presents);
                                                endif;
                                                ?>">
                                            </div>
                                            <div style="text-align: center;" class="col-md-3">
                                                ...
                                                if (isset($worked)) : echo esc_attr($worked);
                                                endif;
                                                ?>">
                                            </div>
                                        <?php }else if (isset($wphrmSalaryAccording['wphrm-according']) && $wphrmSalaryAccording['wphrm-according'] == 'Hourly') { ?>

                                            <div style="text-align: center;" class="col-md-3">
                                                <?php _e('Total Hours', 'wphrm'); ?>
                                                <input form="myform" class="earningDeduction validationonnumber" type="text"  name="wphrm_Hours" id="wphrm_Hours"  value="<?php
                                                if (isset($wphrm_text_info['wphrm_Hours'])) : echo esc_attr($wphrm_text_info['wphrm_Hours']);
                                                endif;
                                                ?>">
                                            </div>
                                            <div style="text-align: center;" class="col-md-3"></div>
                                            <div style="text-align: center;" class="col-md-3"></div>
                                        <?php } else { ?>

                                            <div style="text-align: center;" class="col-md-3">
                                                <?php _e('Total  Working Days', 'wphrm'); ?>
                                                <input style="text-align: center;" form="myform" class="earningDeduction " type="text" name="dayofworking" id="dayofworking"  value="<?php
                                                if (isset($totalofWorkingDays)): echo esc_attr($totalofWorkingDays);
                                                endif;
                                                ?>">
                                            </div>
                                            <div style="text-align: center;" class="col-md-3">
                                                <?php _e('Total Leave Days', 'wphrm'); ?>
                                                <input   form="myform" class="earningDeduction " type="text" name="workleave" id="workleave" value="<?php
                                                if (isset($presents)): echo esc_attr($presents);
                                                endif;
                                                ?>">
                                            </div>
                                            <div style="text-align: center;" class="col-md-3">
                                                <?php _e('Total Days Worked', 'wphrm'); ?>
                                                <input style="width: 100% !important" form="myform"  class="earningDeduction " type="text"  name="dayofworked" id="dayofworked"  value="<?php
                                                if (isset($worked)) : echo esc_attr($worked);
                                                endif;
                                                ?>">
                                            </div>
                                    ...
                                    <div class="col-md-6 col-sm-12 col-xs-12 earnings-table">
                                        <table class="table">
                                            ...
                                            <tbody>
                                                ...
                                                $wphrmEarningfiledskey = $wpdb->get_row("SELECT * FROM $this->WphrmSalaryTable where `employeeID`=$employee_id AND `employeeKey`='$wphrmEarningfiledskeyinformation' AND `date`='$from'");
                                                if ($duplicateMonth != '' && $duplicateFrom != '') {
                                                    $wphrmEarningfiledskey = $wpdb->get_row("SELECT * FROM $this->WphrmSalaryTable where `employeeID`=$employee_id AND `employeeKey`='$wphrmEarningfiledskeyinformation' AND `date`='$duplicateFrom'");
                                                }
                                                if (!empty($wphrmEarningfiledskey)) {
                                                                    ...
                                                                    if ($earningLebal == $earningtype && $earningLebal == $earningamount && $earningtype == $earningamount) {
                                                                        ...
                                                                        <tr class="<?php echo 'removefiled' . esc_attr($j) . 'earningdefaultlabel'; ?>">
                                                                            ...
                                                                            <td  style="padding-right: 0px; float:right;">
                                                                                ...
                                                                                $main = ($stotal * $totalWorked);
                                                                                $sapersant = ($main * $earningamounts) / 100;
                                                                                ?>
                                                                                <input type="text" class="earningDeduction earningcal validationonnumber two-digits" onkeyup="calculateEarningSum()" name="wphrm-earning-value[]" value="<?php echo $sapersant; ?>">    
                                                                            </td>
                                                                            <td style="text-align: center;">
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td  style="padding-right: 0px; float:right;">
                                                            <input type="text" class="earningDeduction earningcal validationonnumber two-digits" onkeyup="calculateEarningSum()" name="wphrm-earning-value[]" value="<?php echo $currents; ?>">    
                                                        </td>
                                                        <td>
                                                        </td>
                                                    </tr>
                                    ...
                                    <div class="col-md-6 col-sm-12 col-xs-12 deductions-table">
                                        <table class="table">
                                            ...
                                            <tbody>
                                                ...
                                                $wphrmDeductionfiledskey = $wpdb->get_row("SELECT * FROM $this->WphrmSalaryTable where `employeeID` = $employee_id AND `employeeKey` = '$wphrmDeductionfiledskeyInformation' AND `date`='$from'");
                                                if ($duplicateMonth != '' && $duplicateFrom != '') {
                                                    $wphrmDeductionfiledskey = $wpdb->get_row("SELECT * FROM $this->WphrmSalaryTable where `employeeID` = $employee_id AND `employeeKey` = '$wphrmDeductionfiledskeyInformation' AND `date`='$duplicateFrom'");
                                                }
                                                                    ...
                                                                    if ($deductionLebal == $deductiontype && $deductionLebal == $deductionamount && $deductiontype == $deductionamount) {
                                                                        ...
                                                                        <tr class=" <?php echo 'removefiled' . esc_attr($j) . 'deductiondefaultlabel'; ?>">
                                                                            ...
                                                                            <td  style="padding-right: 0px; float:right;">
                                                                                ...
                                                                                ?>
                                                                                <input type="text" class="earningDeduction deductioncal validationonnumber two-digits" value="<?php echo round($sapersant, $decimalvalue); ?>" onkeyup="calculateDeductionSum()"  name="wphrm-deduction-value[]" >
                                                                            </td>
                                                                            <td style="text-align: center;">
